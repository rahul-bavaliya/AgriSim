...
from agrisim.api.v1.endpoints.auth import get_current_user
...
def create_field(
    ...
    db_field = field_service.create_field(db=db, field_in=field_in, owner_id=current_user.id)
...
def read_fields(
    ...
    fields = field_service.get_fields_by_owner(db=db, owner_id=current_user.id, skip=skip, limit=limit)
    return ResponseEnvelope(
